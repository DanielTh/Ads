        ...
        <div class="container">
            ...
            <?php
                ...
                try
                {
                    ...
                    {
                        $req = $bdd->prepare("SELECT u1.login, u2.login AS login2, texteMessage, idMessage FROM messages AS m LEFT JOIN users AS u1 ON m.idAuteur1 = u1.idU LEFT JOIN users AS u2 ON m.idAuteur2 = u2.idU WHERE (u1.login = :login AND u2.login = :login2) OR (u1.login = :login2 AND u2.login = :login) ORDER BY idMessage");
                        $req->bindValue(":login", $_SESSION['login']);
